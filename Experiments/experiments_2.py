...
def experiment(task:Task, optimisation_algorithm:str='EA', seed:int=None, stop_on_target:bool=True):
    ...
    elif ndp_params['model'] in ('rewiring_ndp', 'hebbian_ndp'):
        model = RewiringNeuralDevelopmentalProgram(ndp_params)
    elif ndp_params['model'] == 'fixed_mlp':
        model = FixedMLP(ndp_params)
        evaluate_ndp = task.evaluate_fixed_mlp
    else:
        raise ValueError('Model on task should be standard_ndp, rewiring_ndp or fixed_mlp.')
    ...
    if (ndp_params['initial_node_state_mode'] == 'coevolve') and (ndp_params['model'] != 'fixed_mlp'):
        if ndp_params['model'] in ('rewiring_ndp', 'hebbian_ndp'):
    ...
    if optimisation_algorithm == 'CMA':
        ...
        optimiser = EvolutionaryAlgorithm(
            ...
            population_size = ndp_params['population_size'],
            max_iterations = ndp_params['generations'], 
            max_stagnment = ndp_params['stagnant_generation'],
            objective_function = evaluate_ndp,
            run_in_parallel = run_in_parallel,
            cores = cores,
            stop_on_target = stop_on_target,
            crossover_probability = ndp_params['crossover_probability'],
            mutation_probability = ndp_params['mutation_probability'],
            sbx_eta_min = ndp_params['sbx_eta_min'],
            sbx_eta_max = ndp_params['sbx_eta_max'],
            mutation_eta_min = ndp_params['mutation_eta_min'],
            mutation_eta_max = ndp_params['mutation_eta_max'],
            eta_schedule_iterations = ndp_params['eta_schedule_iterations'],
