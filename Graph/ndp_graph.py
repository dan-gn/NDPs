...
class Graph:
    ...
    def add_edge(self, input:int, output:int, weight:float = 1.0):
        self.edges[(input, output)] = weight if self.weighted_graph_flag else None
    ...
    def get_neighbors(self, node_id:int) -> list[int]:
        neighbors = []
        for (input, output), _ in self.edges.items():
            if input == node_id:
                neighbors.append(output)
            elif output == node_id:
                neighbors.append(input)
        return neighbors
    ...
    def summary(self):
        ...
        print('Edges')
        print('-----------------')
        for (input, output), weight in self.edges.items():
            print(f'input = {input}, output = {output}, weight = {weight}')
        print('\n')
