...
class HebbianNeuralDevelopmentalPrograms(NeuralDevelopmentalProgram):
    ...
    def generate_initial_seed_graph(self) -> Graphnx:
        ...
        if self.initial_node_state_mode == 'coevolve':
            ...
            network_seed = coevolved_array[0]
        ...
        graph.add_sparese_edges(input_nodes, output_nodes, density=1.0, rng=rng)

        # Add edges from inputs to hidden nodes (sparse)
        graph.add_sparese_edges(input_nodes, hidden_nodes, density=density, rng=rng)

        # Add edges from hidden to hidden nodes (sparse)
        graph.add_sparese_edges(hidden_nodes, hidden_nodes, density=density, rng=rng)

        # Add edges from hidden to output nodes (sparse)
        graph.add_sparese_edges(hidden_nodes, output_nodes, density=density, rng=rng)
